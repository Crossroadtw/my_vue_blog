  ...
  <div>
    ...
    <div class="bubbels">
      <div v-for="(item,index) in bubbles" v-bind:key="index" class="bubble"
           v-bind:style="item.color+item.width+item.height+item.left+item.delay+item.duration">
      </div>
    </div>
    <!-- 上升气泡   stop-->
  </div>
...
export default {
  data () {
    const text = ['python', '二刺猿', '楪祈', '动漫', 'C/C++', 'Vue-cli', '全栈']
    const loves = []
    const bubbles = []
  ...
  methods: {
    ...
    createBubble () {
      // const left = [10, 20, 35, 50, 55, 65, 75, 80, 70, 85, 77]
      const wh = [40, 20, 50, 80, 35, 45, 25, 80, 15, 50, 77]
      const duration = [8, 5, 10, 7, 6, 8, 7, 6, 9, 5, 8]
      const delay = [0, 1, 1, 0, 0, 0, 2, 1, 0, 3, 2]
